...
from .settings_widgets import LineEditSetting
...
class AutoSaveSettings(SettingsTab):
    def __init__(self):
        super().__init__(_("Metadata and Downloads"))
        S = Settings
        
        # Connecter les signaux pour la mise à jour en temps réel
        self._metadata_widgets = {}
        
        # Metadata Display Section
        metadata_label = QLabel(_("Metadata Display"))
        metadata_label.setStyleSheet("font-weight: bold; font-size: 14px; margin-top: 10px;")
        self._layout.addWidget(metadata_label)
        self.add("show_metadata_interface", SwitchSetting(S._show_metadata_interface, parent=self))
        self.add("metadata_font_size", SpinBoxSetting(S._metadata_font_size, self, 8, 24))
        
        # Info text for font size
        font_info = QLabel(_("Note: Font size changes apply immediately to the metadata panel"))
        font_info.setStyleSheet("color: #888; font-size: 12px; font-style: italic; margin-left: 10px;")
        self._layout.addWidget(font_info)
        
        self.add("metadata_widget_height", SpinBoxSetting(S._metadata_widget_height, self, 120, 600))
        
        # Info text for widget height
        height_info = QLabel(_("Note: Widget height changes apply immediately to the metadata panel"))
        height_info.setStyleSheet("color: #888; font-size: 12px; font-style: italic; margin-left: 10px;")
        self._layout.addWidget(height_info)
        
        # Info about interface buttons
        interface_info = QLabel(_("Tip: You can also use the A+/A- and +/- buttons in the metadata panel for quick adjustments"))
        interface_info.setStyleSheet("color: #888; font-size: 12px; font-style: italic; margin-left: 10px; margin-top: 5px;")
        self._layout.addWidget(interface_info)
        
        # Stocker les widgets pour la mise à jour en temps réel
        self._metadata_widgets['show_interface'] = self._widgets['show_metadata_interface']
        self._metadata_widgets['font_size'] = self._widgets['metadata_font_size']
        self._metadata_widgets['widget_height'] = self._widgets['metadata_widget_height']
        
        # Connecter les signaux pour la mise à jour en temps réel
        self._metadata_widgets['font_size'].value_changed.connect(self._update_metadata_font_size)
        self._metadata_widgets['widget_height'].value_changed.connect(self._update_metadata_height)
        self._metadata_widgets['show_interface'].value_changed.connect(self._update_metadata_visibility)
        
        self._layout.addSpacing(10)
        
        # Automatic Downloads Section
        downloads_label = QLabel(_("Automatic Downloads"))
        downloads_label.setStyleSheet("font-weight: bold; font-size: 14px; margin-top: 10px;")
        self._layout.addWidget(downloads_label)
        self.add("auto_save_generated", SwitchSetting(S._auto_save_generated, (_("Enabled"), _("Disabled")), self))
        
        # Description for folder field
        folder_desc = QLabel(_("Folder where generated images will be automatically saved"))
        folder_desc.setStyleSheet("color: #888; font-size: 12px; font-style: italic; margin-left: 10px;")
        self._layout.addWidget(folder_desc)
        
        # Folder selection with browse button
        folder_layout = QHBoxLayout()
        folder_layout.setSpacing(5)
        
        self._folder_edit = LineEditSetting(S._auto_save_folder, self)
        # S'assurer que la valeur est chargée au démarrage
        self._folder_edit.value = settings.auto_save_folder
        folder_layout.addWidget(self._folder_edit)
        
        self._browse_button = QPushButton("📁", self)
        self._browse_button.setFixedSize(30, 30)
        self._browse_button.setToolTip(_("Browse for folder"))
        ...
        # Actions Section
        self._layout.addSpacing(10)
        actions_label = QLabel(_("Actions"))
        actions_label.setStyleSheet("font-weight: bold; font-size: 14px; margin-top: 10px;")
        self._layout.addWidget(actions_label)
        
        # Bouton pour télécharger toutes les images de l'historique
        self._download_all_button = QPushButton(_("Download all images from history"), self)
        self._download_all_button.clicked.connect(self._download_all_images)
        self._layout.addWidget(self._download_all_button)
        
        self._layout.addStretch()

    def _read(self):
        """Read settings from the settings object"""
        # Les paramètres sont automatiquement lus par le système SettingsTab
        # grâce aux widgets ajoutés avec self.add()
        
        # S'assurer que le dossier est correctement chargé
        if hasattr(self, '_folder_edit'):
            self._folder_edit.value = settings.auto_save_folder

    def _write(self):
        """Write settings to the settings object"""
        # Les paramètres sont automatiquement écrits par le système SettingsTab
        # grâce aux widgets ajoutés avec self.add()
        
        # Mise à jour en temps réel des paramètres de métadonnées
        if hasattr(self, '_widgets'):
            if 'metadata_font_size' in self._widgets:
                font_size = self._widgets['metadata_font_size'].value
                settings.metadata_font_size = font_size
                
            if 'metadata_widget_height' in self._widgets:
                widget_height = self._widgets['metadata_widget_height'].value
                settings.metadata_widget_height = widget_height
                
            if 'show_metadata_interface' in self._widgets:
                show_interface = self._widgets['show_metadata_interface'].value
                settings.show_metadata_interface = show_interface

    def _update_metadata_font_size(self):
        """Update metadata font size in real time"""
        if hasattr(self, '_metadata_widgets') and 'font_size' in self._metadata_widgets:
            font_size = self._metadata_widgets['font_size'].value
            settings.metadata_font_size = font_size
            settings.save()
            # Émettre un signal pour notifier les autres composants
            settings.changed.emit('metadata_font_size', font_size)

    def _update_metadata_height(self):
        """Update metadata widget height in real time"""
        if hasattr(self, '_metadata_widgets') and 'widget_height' in self._metadata_widgets:
            widget_height = self._metadata_widgets['widget_height'].value
            settings.metadata_widget_height = widget_height
            settings.save()
            # Émettre un signal pour notifier les autres composants
            settings.changed.emit('metadata_widget_height', widget_height)

    def _update_metadata_visibility(self):
        """Update metadata interface visibility in real time"""
        if hasattr(self, '_metadata_widgets') and 'show_interface' in self._metadata_widgets:
            show_interface = self._metadata_widgets['show_interface'].value
            settings.show_metadata_interface = show_interface
            settings.save()
            # Émettre un signal pour notifier les autres composants
            settings.changed.emit('show_metadata_interface', show_interface)
    ...
    def _browse_folder(self):
        ...
        # Utiliser le chemin actuel s'il existe, sinon le dossier utilisateur
        current_folder = settings.auto_save_folder
        if current_folder and current_folder != "generated_images":
            ...
            # Si c'est la valeur par défaut, commencer dans le dossier utilisateur
            start_folder = str(Path.home())
        ...
        if folder_path:
            self._folder_edit.value = folder_path
            settings.auto_save_folder = folder_path
            settings.save()

    def _download_all_images(self):
        """Télécharge toutes les images de l'historique actuel"""
        from ..root import root
        from ..persistence import _auto_save_images_from_history
        
        if not root.active_model:
            QMessageBox.warning(self, _("Erreur"), _("Aucun modèle actif trouvé"))
            return
            
        if not settings.auto_save_folder:
            QMessageBox.warning(self, _("Erreur"), _("Veuillez d'abord configurer un dossier de sauvegarde"))
            return
            
        try:
            count = _auto_save_images_from_history(root.active_model)
            QMessageBox.information(self, _("Succès"), 
                _("{} images ont été téléchargées dans le dossier configuré").format(count))
        except Exception as e:
            QMessageBox.critical(self, _("Erreur"), 
                _("Erreur lors du téléchargement : {}").format(str(e)))
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, server: Server):
        ...
        create_list_item(_("Metadata and Downloads"), self.autosave)
