...
class Image:
    ...
    def save(self, filepath: Union[str, Path], metadata: dict[str, str] = None):
        ...
        try:
            if metadata and fmt.value[0] == "png":
                # Pour les PNG, on peut ajouter des métadonnées via QImageWriter
                writer = QImageWriter(file, QByteArray(b"png"))
                writer.setQuality(fmt.value[1])
                
                # Ajouter les métadonnées
                for key, value in metadata.items():
                    writer.setText(key, str(value))
                
                result = writer.write(self._qimage)
                if not result:
                    raise Exception(f"Failed to write image with metadata: {writer.errorString()}")
            else:
                # Pour les autres formats ou sans métadonnées, utiliser la méthode normale
                self.write(file, fmt)
        finally:
            file.close()
