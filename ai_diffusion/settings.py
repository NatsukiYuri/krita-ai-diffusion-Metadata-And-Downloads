...
class Settings(QObject):
    ...
    _auto_save_generated = Setting(
        _("Auto-save Generated Images"),
        False,
        _("Automatically save all generated images to a folder"),
    )

    auto_save_folder: str
    _auto_save_folder = Setting(
        _("Auto-save Folder"),
        str(user_data_dir / "generated_images"),
        _("Folder where generated images will be automatically saved"),
    )

    show_metadata_interface: bool
    _show_metadata_interface = Setting(
        _("Show Metadata Interface"),
        True,
        _("Display the metadata interface below the history"),
    )

    metadata_font_size: int
    _metadata_font_size = Setting(
        _("Metadata Font Size"),
        15,
        _("Font size for the metadata display interface"),
    )

    metadata_widget_height: int
    _metadata_widget_height = Setting(
        _("Metadata Widget Height"),
        350,
        _("Height of the metadata display widget"),
    )
    ...
    def __setattr__(self, name: str, value):
        if name in self._values:
            if self._values[name] != value:
                ...
                # Language changes are handled in settings only
        else:
